...
//Daniel Malmer
//5/1/09
//edited various places for genetic algorithm
//noted changes with a DM
...
class PacmanGame {
public:
    int RunGame(string moves, bool display);
    void SeedGhostMoves();
    int GetCycles();
    void IntroScreen();
    void CheckCollision(bool display);
    ...
    void MainLoop(string moves, bool display);
    void MoveGhosts();
    void MovePacman();
    void PauseGame();
};
...
char LevelFile[50] = "level__.dat"; //Locations of default levels
int FreeLife = 1000; //Starting points for free life
int Points = 0; //Initial points
//int Lives = 0; //Number of lives you start with. DM - removed lives system
int HowSlow = 3; //How slow vulnerable ghost move, default = 3
int SpeedOfGame = 100000; //How much of a delay is in the game, DM - changed so its based on useconds
...
//int tleft = 0; //How long left for invincibility, DM - changed so its based on ticks, not actual time

//DM - added my own global variables
bool Alive = true;
int ghostMoves[1000];
int ghostIndex;
int TicksInvincible = 0;
int SlowerGhosts = 0; //DM - changed to a global variable, fixed major bug resulting in different games for the same set of moves

//DM - added moves being passed in
int PacmanGame::RunGame(string moves, bool display) {
	...
	//int j = 0;
	//DM - removed to allow testing
	//srand( (unsigned)time( NULL ) );

	InitCurses();
	//DM - only draw if in display mode
	if (display) {
	    CheckScreenSize();
	    CreateWindows(29, 28, 1, 1);
	}
	//DM - removed, don't need
...
//DM - seed ghosts moves so they run the same each time
void PacmanGame::SeedGhostMoves() {
    for (int i = 0; i < 1000; i++) {
	ghostMoves[i] = rand() % 4;
    }
}

void PacmanGame::CheckCollision(bool display) {
	int a = 0;
	for(a = 0; a < 4; a++) {
		//Collision
...
void PacmanGame::CheckScreenSize() {
...
void PacmanGame::CreateWindows(int y, int x, int y0, int x0) {
	win = newwin(y, x, y0, x0);
	status = newwin(3, 27, 29, 1);
}

void PacmanGame::Delay() {
    //DM - changed delay so it uses useconds now
    usleep(SpeedOfGame); 
    /*
...
void PacmanGame::ExitProgram() {
	endwin();
	//DM - removed message and exit
	//printf("%s\n", message);
	//exit(0);
	//DM - used to get out of loops
	Alive = false;
}

//DM - added char parameter
void PacmanGame::GetInput(char move) {
  ...
  /* DM - removed input
	...
	switch (ch) {
  */
    //DM - move pacman based on char passed in
        switch (move) {
...
void PacmanGame::LoadLevel(char levelfile[100]) {
	...
	Dir[4][0] =  0; Dir[4][1] = -1;
        ...
        if(!(fin)) { strcat(error, levelfile); ExitProgram(); }
	...
	//DM - close the file when done, fixed seg fault bug
	fclose(fin);

}

void PacmanGame::MainLoop(string moves, bool display) {

        //DM - only draw window in in display mode
        if (display) {
	    DrawWindow();
	    wrefresh(win);
	    wrefresh(status);
	...
	char move;
	int i;
	//DM - loops until you beat the level or die
	for (i = 0; Food > 0 && Alive; i = (i + 1) % moves.length()) {
	  //DM - add pacman direction
	  GetInput(moves.at(i));
	  MovePacman();	 
	  //DM - only draw if in display mode
	  if (display) { 
	...
	//DM - added points for winning the level
	if (Food == 0) {
	    Points += 1000;
	}
...
void PacmanGame::MoveGhosts() {
	...
	//DM annotation - when SlowerGhosts EQUALS HowSlow, ghost doesn't move
	//so when HowSlow = 3, the ghost won't move every third tick
	...
	for(a = 0; a < 4; a++) {
		...
		do {
			//Decide direction
		        //DM - removed unique randomness, now use seeded ghost moves
		        //b = (int)(rand() / (1625000000 / 4));
		        b = ghostMoves[ghostIndex];
			ghostIndex = (ghostIndex + 1) % 1000;
...
void PacmanGame::MovePacman() {
    //DM - removed, changed invincibility to run based on ticks, not seconds
    //static int itime = 0;
	...
	switch (Level[Loc[4][0]][Loc[4][1]]) {
		...
		case 3:	//PowerUp
			...
			//if(GhostsInARow == 0) 
			    GhostsInARow = 1;
			//DM - changed invicibility
			//itime = time(0);
			TicksInvincible = 40;
			break;
	}

	//DM - changed how invicibility works, now based on ticks instead of time
	//Is he invincible?
	if (Invincible == 1) {
